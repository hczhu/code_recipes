...
template <typename T> istream &operator>>(istream &in, vector<T> &vect) {
  vect.clear();
  int n;
...
template <typename T> inline vector<T> erase(vector<T> table, int ind) {
  assert(ind < table.size());
  table.erase(table.begin() + ind);
  return table;
}
template <typename T> vector<T> unique(vector<T> table) {
  SORT(table);
...
//************************************************
//**********Number functions*****************
template <typename T> T __gcd(T n, T m, T &a, T &b) {
  T a1 = 0, b1 = 1;
  a = 1, b = 0;
...
template <typename T> double length(complex<T> p) {
  return sqrt(1.0 * p.x() * p.x() + 1.0 * p.y() * p.y());
}
template <typename T> double dnorm(complex<T> p) {
...
template <typename T> llint llcross(complex<T> p1, complex<T> p2) {
  return (llint)p1.x() * (llint)p2.y() - (llint)p2.x() * (llint)p1.y();
}
template <typename T> int sigcross(complex<T> p1, complex<T> p2) {
...
template <typename T> double dcross(complex<T> p1, complex<T> p2) {
  return 1.0 * p1.x() * p2.y() - p2.x() * p1.y();
}
template <typename T> double ddot(complex<T> p1, complex<T> p2) {
...
template <typename T> class pointLess : public less<complex<T>> {
public:
...
public:
  static void set_para(T (*add)(const T &, const T &),
                       T (*mul)(const T &, const T &), const T &zero,
                       const T &one) {
    _add = add;
    _mul = mul;
    _zero = zero;
  ...
  static Matrix<T> ONE(int n, int m) {
    Matrix<T> res(n, m, _zero);
    for (int i = 0; i < n; i++)
      res[i][i] = _one;
    return res;
  };
...
template <typename T> Matrix<T> Matrix<T>::power(llint s) const {
...
template <typename T> Matrix<T> Matrix<T>::power_sum(llint s) const {
  assert(this->size == this->at(0).size());
  if (s == 1)
    return *this;
  if (s == 0)
    return Matrix<T>::ZERO(this->size(), this->size());
  llint mid = s / 2;
  Matrix<T> low = power_sum(mid);
  Matrix<T> high;
  if (s - mid > mid)
    high = low + power(s - mid);
  else
    high = low;
  high = high * power(mid);
  return low + high;
...
template <typename T> Matrix<T> Matrix<T>::operator*(const Matrix &b) const {
...
Matrix<T> Matrix<T>::operator+(const Matrix &other) const {
  ...
  if (n == 0)
    return Matrix<T>();
  const int m = other[0].size();
  Matrix<T> res(n, m);
  for (int i = 0; i < n; i++)
    for (int j = 0; j < m; j++)
      res[i][j] = _add((*this)[i][j], other[i][j]);
  return res;
}
//************************************************
// binary index tree
template <typename T> void update(int pos, T add, T tree[], int n) {
  while (pos <= n)
    tree[pos] += add, pos += Low(pos);
}
template <typename T> T query(int pos, T tree[]) {
  T res = 0;
  while (pos)
    res += tree[pos], pos ^= Low(pos);
  return res;
}
...
#define get_lr()                                                               \
  int mid = (low + high) >> 1, ll = (ind << 1) ^ 1, rr = (ind << 1) + 2;
...
void update_interval(llint a, llint b, llint &lower, llint &upper) {
  ...
  if (a > 0) {
    if (b >= 0)
      ...
      upper = min(upper, (b - a + 1) / a);
  } else {
    a = -a;
    b = -b;
    if (b >= 0)
      lower = max(lower, (b + a - 1) / a);
    else
      lower = max(lower, b / a);
  }
}
template <typename T, int N>
void init_bino(T com[N][N], bool take_mod = false,
               long long int mod = 1000000000000000000LL) {
  if (take_mod) {
    assert(mod > 0);
    com[0][0] = 1 % mod;
    for (int i = 1; i < N; i++) {
...
llint eval(llint a, llint b, char op) {
  switch (op) {
  case '-':
    return a - b;
  case '+':
...
int main() {
  ...
  for (;;) {
    ...
    if (isdigit(ch)) {
      ...
      while (!operat.empty() && priority[operat.top()] >= priority[ch]) {
        if (operat.top() == '(') {
          if (ch == ')')
            operat.pop();
          break;
