...
def printTree(seg, indents="  "):
    print(f"{indents}[{seg.l}, {seg.r}) = max({seg.max_row_seats}) sum({seg.sum_row_seats})")
...
def constructSegmentTree(seg, m):
    ...
    l_seg = Segment(l=seg.l, r=mid, max_row_seats=m, sum_row_seats=m*(mid - seg.l))
    r_seg = Segment(l=mid, r=seg.r, max_row_seats=m, sum_row_seats=m*(seg.r - mid))
